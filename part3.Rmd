...
This is because the size factors may no longer be centred at unity after subsetting, especially if the quantities of spike-in RNA differ between batches.
If one batch contained less spike-in RNA, normalization with globally-centred size factors would push up the average abundance for spike-in transcripts in that batch.
This means that the trend would be shifted along the x-axis and would not accurately capture the technical component for each gene.
