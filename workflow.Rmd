...
The process is the same as that used to identify marker genes in the brain data set, except that the design matrix is constructed from the pre-defined variables rather than the empirical clusters.
...
Applying a DE analysis on the single-cell counts will treat the cells within each group as independent replicates.
...
Any uncontrolled variability in processing for one plate will cause a consistent change in expression for all cells on that plate, i.e., a plate effect [@hicks2015xxx].
