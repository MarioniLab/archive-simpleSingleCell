...
Size factors can be computed with several different approaches, e.g., using the `estimateSizeFactorsFromMatrix` function in `r Biocpkg("DESeq2")`, or through the `calcNormFactors` function in `r Biocpkg("edgeR")` (the latter requires an additional multiplication by the library size).
However, single-cell data can be problematic for these methods due to the dominance of low and zero counts.
