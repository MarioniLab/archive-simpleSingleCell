...
Filtering can also be performed to remove genes that are known to be uninteresting.
Here, we remove mitochondrial genes as we are primarily interested in the regulation of nuclear genes.
Other candidates for removal include pseudo-genes or ribosomal proteins that are generally uninteresting but may affect downstream analyses.
...
We also remove mitochondrial genes prior to downstream analyses.
This is particularly important in this data set, as differences in mitochondrial RNA content across cell types can have a substantial impact, e.g., on HVG detection and clustering.
...
To identify HVGs, we fit a trend to the technical variances for the spike-in transcripts as previously described.
We compute the biological component of the variance for each gene by subtracting the fitted trend from the total variance.
However, this is complicated by the low coverage of the spike-in transcripts in this data set.
Indeed, we must remove some cells with very low (or even zero) total spike-in counts before we can compute the variance.
...
The plot below suggests that the trend is fitted well to the technical variances.
...
The top 200 HVGs can be tested for significant correlations.
...
Correlated HVGs are used to construct a heatmap to inspect for subpopulations.
The plot below shows systematic differences between groups of cells with distinct patterns of expression.
This is consistent with the presence of well-defined subpopulations that are also recovered by clustering.
...
This is done using the likelihood ratio test (LRT) for each comparison.
This is illustrated below for cluster 2.
...
We save this list of genes for further examination.
We also examine the expression profile for the top set of genes to verify that the DE is not being driven by outlier cells.
The heatmap below indicates that all of the markers are strongly upregulated across the majority of cells in cluster 2 compared to the other clusters.
