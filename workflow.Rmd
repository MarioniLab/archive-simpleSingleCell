...
plotPCA(sce, exprs_values="norm_exprs") + fontsize
```
...
library(Rtsne)
set.seed(100)
out5 <- plotTSNE(sce, exprs_values="norm_exprs", perplexity=5) + fontsize + ggtitle("Perplexity = 5")
out10 <- plotTSNE(sce, exprs_values="norm_exprs", perplexity=10) + fontsize + ggtitle("Perplexity = 10")
out20 <- plotTSNE(sce, exprs_values="norm_exprs", perplexity=20) + fontsize + ggtitle("Perplexity = 20")
...
As the trend can be rather non-linear, some adjustment of the span for loess curves (or the degrees of freedom for polynomial curves) may be required to obtain a satisfactory fit.

```{r}
var.fit <- trendVar(sce, trend="loess", span=0.25)
```
...
However, the cost of this robustness is the need to fit a rather complex mean-variance relationship.
...
H2-Aa is a component of the class II major histocompatibility complex and may be indicative of B-cell contamination [@heng2008immunological].
Fos and Jun may also be relevant as they are involved in cell proliferation [@angel1991role].
...
set.seed(100)
out.pca <- plotPCA(sce, exprs_values="norm_exprs", feature_set=chosen) + fontsize
out.tsne <- plotTSNE(sce, exprs_values="norm_exprs", feature_set=chosen) + fontsize
...
```{r}
my.dist <- dist(t(heat.vals[chosen,]))
