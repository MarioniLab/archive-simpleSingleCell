...
```{r hvgplothsc, fig.cap="Variance of normalized log-expression values for each gene in the HSC data set, plotted against the average log-expression. The red line represents the mean-dependent trend in the technical variance of the spike-in transcripts (also highlighted as red points)."}
...
pca1 <- plotPCA(sce, exprs_values="norm_exprs", colour_by="tissue") + fontsize
pca2 <- plotPCA(sce, exprs_values="norm_exprs", colour_by="sex") + fontsize
...
set.seed(100)
tsne1 <- plotTSNE(sce, exprs_values="norm_exprs", colour_by="tissue") + fontsize
set.seed(100)
tsne2 <- plotTSNE(sce, exprs_values="norm_exprs", colour_by="sex") + fontsize
...
(We separate cells by tissue to avoid picking up the tissue effect again.)
...
pca1 <- plotPCA(sce[,sce$tissue=="sscortex"], exprs_values="norm_exprs", 
    colour_by="chip") + fontsize + ggtitle("Cortex")
pca2 <- plotPCA(sce[,sce$tissue!="sscortex"], exprs_values="norm_exprs", 
...
However, for the purposes of this workflow, we will consider the tissue of origin as being an uninteresting confounding effect.
As such, we will block on the tissue of origin in all of our downstream analyses.
