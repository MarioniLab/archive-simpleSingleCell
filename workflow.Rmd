...
Correlated HVGs are used to construct a heatmap that can be inspected for subpopulations.
However, before we do so, we use the `removeBatchEffect` function in the `r Biocpkg("limma")` package to remove the tissue effect.
This ensures that the tissue of origin will not dominate the visualization.
...
The empirical clusters also correspond roughly to the visual subpopulations. 
...
This is done using the likelihood ratio test (LRT) for each comparison, and is demonstrated below for cluster 2.
...
In general, `r Biocpkg("edgeR")` will automatically protect against outlier expression patterns, as any outliers will inflate the dispersion and increase the _p_-value for the affected gene.
