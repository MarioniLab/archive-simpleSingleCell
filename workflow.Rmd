...
```{r}
incoming <- read.table("~/AaronDocs/Research/DataFiles/ESpresso/counttable_es.csv", 
...
Some data sets contain pre-defined biological groups in the experimental design, e.g., the tissue and sex in the brain data set.
If differences between these pre-defined groups are of interest, a DE analysis can be performed with packages like `r Biocpkg("edgeR")` or `r Biocpkg("DESeq2")`.
Briefly, a design matrix is constructed containing the pre-defined groupings for all cells.
This design matrix and the matrix of single-cell counts can then be processed with standard DE analysis pipelines.
This is similar to the identification of marker genes for the brain data set, except that pre-defined variables are used in the design matrix instead of empirical clusters.
