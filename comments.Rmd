...
What happens if each cell has a different dropout rate?
This should not affect scaling normalization, which only cares about equating the expected expression.
Whether technical differences in expression occur in the zero or non-zero components is irrelevant to the procedure.
The distribution of expression only matters insofar as the robust average estimators are accurate.
It does, however, pose problems in how the size factor is to be split up in a ZI model; how much of it goes to the proportion term, and how much of it changes the non-zero mean?

You could theoretically use the abundance-dependent trend to empirically determine how the size factor might change the proportion.
However, I'm not convinced that this trend justifies the need to model the dropouts separately.
You can get a natural sigmoidal curve without requiring an explicit zero inflation term in a NB distribution.
Here, scaling of the values would work just as well to account for changes in total RNA content.
...
More generally, the difficulty with dealing with dropouts is that you don't know whether a zero count is a true dropout or due to a genuine lack of expression.
It's possible to figure this out based on expression in other cells, e.g., a zero is more likely to be a dropout if expression is non-zero in other cells.
This assumes that the true expression in each cell is sampled from the same distribution for each gene.
The assumption can be weakened by only using "similar" cells to impute dropouts for each cell.
However, this shifts the burden to how similarity is defined - the assumption would be sensitive to errors/uncertainty in the similarity metric.

In any case, a variety of strategies can be used to actually handle the dropouts on a per-observation basis:

- Imputing is most popular, but seems somewhat circular if you try to impute from the same data (rather than using external data, e.g., as in variant calling).
This is particularly true if you impute from similar cells - downstream clustering on the imputed counts would just support the initial similarities.
- Censoring suspected dropouts avoids the need to explicitly guess what the non-zero value would be.
However, it requires a (fairly arbitrary) specification of the threshold at which a dropout is distinguished from a genuine zero.
- Downweighting seems most gradated but it depends on a sensible definition of the weights.
I would probably define the weights as the probability that an observation is not a dropout in a ZI model, but most downstream tools won't handle that.
