...
The solution would be to model the spread around the trend with an F-distribution, but there's not enough spike-ins to do that precisely.
...
However, if we want to be conservative, then we look for genes that are expressed consistently _in spite of_ the technical noise.
This favours DE genes in which we know that expression is consistent, at the expense of genes where we don't know either way due to the noise.
To do this, we take the log-expression values at face value and just use _limma_ without further work.
