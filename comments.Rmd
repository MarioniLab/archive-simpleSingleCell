...
This avoids the above issue, but it implicitly upweights HVGs with low technical noise and small (non-zero) biological components.
...
Stochastic zeroes due to low library sizes don't really mess up the _expected_ clustering, as long as normalization is properly performed.
After normalization, the cells should just be scattered around the true mean expression, regardless of how many stochastic zeroes are present.
(But see my comments about the log-transformation in the factor analysis section.)
However, zeroes may distort rank-based clustering, which is why you should filter for high-abundance genes before running `quickCluster`.
This is probably driven by a shift in the population center in rank space when there are ties, and is illustrated in the example below.
...
x <- cbind(a, b)
table(rep(1:2, each=ncells), quickCluster(x, min.size=100)) # on ranks
```
...
The trade-off is that it is sensitive to violations of the model assumptions, and there are no unique solutions; it may also be slower, though PCA isn't exactly fast either.
...
In practice, this doesn't really matter - if the mean log-values differ with respect to the size factor, it implies that the variance at one size factor is much larger.
