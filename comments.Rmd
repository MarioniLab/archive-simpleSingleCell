...
With the HSC and brain data sets, the variance explained due to the (log-)total count is around 1% for most genes, and well under 5-10% in general (via `plotExplanatoryFactors`).
...
The better way to tell is to look at the spike-ins, where we can attribute non-linearity with respect to abundance as technical bias.
We plot the total count of low-abundance spike-ins against that of the high-abundance spike-ins.
If low- and high-abundance transcripts differ in their behaviour, there should be a trend here that does not have an intercept of 1.
In other words, a doubling for high-abundance transcript will not produce a doubling for low-abundance transcripts.
Fortunately, we see intercepts fairly close to 1 in both data sets, so I'm pretty happy with this. 
...
sp <- spikes(sce)
middle <- median(ab)
log.high <- log10(colSums(sp[ab>middle,]))
log.low <- log10(colSums(sp[ab<middle,]))
summary(log.high)
summary(log.low) # 2 order of magnitude difference
drop <- is.finite(log.low)
lm(log.low[drop] ~ log.high[drop])
```
