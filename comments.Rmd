...
In the HVG data set, an average count of 1 corresponds to a maximum variance of around 1 (as in, the maximum possible variance, given that counts must be positive). 
...
```{r}
plot(0, 0, type="n", xlab="Log mean", ylab="Variance", xlim=c(0, 1), ylim=c(0, 1.5))
for (ave.count in 1:20/20) {
    lmus <- log2(ave.count/(1-x)+1) * (1-x) + log(0+1)*x
    lvars <- (log2(0+1)-lmus)^2 * x + (log2(ave.count/(1-x)+1)-lmus)^2*(1-x)
    points(lmus, lvars)
}
lines(c(0, 0.2, 0.4), c(0, 1, 2), col="red", lwd=2) # Technical noise trend.
```

The reasoning in the brain data is slightly different.
In this case, the technical noise is lower, but so are the absolute sizes of the maximum variances.
Here, we set a filter of 0.2 to remove genes where the maximum variance is too small to be of interest, even if it is greater than the technical trend.
A rule of thumb is that the max variance should be above 0.5 (see below), though we're a bit more relaxed just in case a low-variance HVG turns out to be useful later.

```{r}
plot(0, 0, type="n", xlab="Log mean", ylab="Variance", xlim=c(0, 0.5), ylim=c(0, 0.5))
for (ave.count in 1:4/20) {
    lmus <- log2(ave.count/(1-x)+1) * (1-x) + log(0+1)*x
    lvars <- (log2(0+1)-lmus)^2 * x + (log2(ave.count/(1-x)+1)-lmus)^2*(1-x)
    points(lmus, lvars)
}
lines(c(0, 0.3, 1), c(0, 0.3, 0.6), col="red", lwd=2) # Technical noise trend.
```
