...
However, as a result of the robustness to outliers, detection power is reduced for HVGs driven by rare subpopulations.
CV2-based methods do better, as shown below where HVGs corresponding to rare subpopulations are detected with lower mean expression in those cells.
...
min(which(out$FDR[chosen] <= 0.05))

# Log-variances detects it at a count of 100 (around 700 if you also require biological variances above 0.5). 
...
to.run <- seq_len(500)
my.means <- means[to.run]
subcounts <- matrix(rnbinom(length(to.run)*nsamples, mu=my.means, size=1/dispersions[to.run]), ncol=nsamples)
subcounts[,1:10] <- 0
chosen <- nrow(counts) + to.run
counts <- rbind(counts, subcounts)
is.spike <- c(is.spike, logical(length(to.run)))
...
And obviously, with CV^2^ you pick up a lot of genes expressed in few cells, which can be a pain for downstream analyses.
...
It also disrupts clustering of subtle subpopulations and trajectory identification.
...
# CV2 generates more noise.
out <- technicalCV2(counts, is.spike, sf.cell=rep(1, nsamples), sf.spike=rep(1, nsamples))
lcounts <- log2(counts+1)
sig.out <- lcounts[out$FDR <= 0.05 & !is.na(out$FDR),]
pr.out <- prcomp(t(sig.out))

# Log-variances detects the manifold clearly.
fit <- trendVar(lcounts[is.spike,])
ref <- decomposeVar(lcounts, fit)
sig.ref <- lcounts[ref$FDR <= 0.05 & !is.na(ref$FDR) & ref$bio > 0.5,]
