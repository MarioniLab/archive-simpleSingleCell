...
(On th other hand, if you don't scale, then the selection of genes will be dominated by the mean-variance relationship.)

Detecting significant correlations beforehand is also useful for minimizing error in the identified loading vector.
The example below considers a small subset of correlated genes in the midst of uncorrelated noise (biological or technical).
Clearly, the correct placing is more accurately recovered when only the correlated genes are used.

```{r}
par(mfrow=c(1,2))
loc <- 1:100/10 # True placement of cells
a1 <- matrix(jitter(rep(loc, 10)), nrow=10, byrow=TRUE) # Correlated genes
x1 <- prcomp(t(a1), scale=TRUE)
plot(x1$x[,1]) # Should be on the diagonal
a2 <- rbind(a1, matrix(runif(100000), ncol=100)) # Adding uncorrelated noise
