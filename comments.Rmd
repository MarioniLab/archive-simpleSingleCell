...
This results in loss of resolution, but this is fine as the point of pre-clustering is to reduce DE genes rather than to resolve biological clusters.
...
# It gets better after some aggressive filtering on the mean.
keep <- means > 90
obs <- quickCluster(x[keep,], min.size=50)
table(truth, obs) 
```
