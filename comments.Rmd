...
It is logical to remove low-quality cells before computing the averages, otherwise they would be distorted by empty cells or cells with 100% spike-ins.
One _could_ argue that some cells become low quality after filtering on the genes.
...
Loss of an inordinate number of expressed features in some cells relative to others would only be observed for DE genes.
If most genes are not DE, they should be filtered at an equal rate for all cells.
This suggests that outliers for the number of expressed features are unlikely to be suddently generated after gene filtering.
...
Modest/high-abundance genes with big changes in expression are also more likely to actually contribute to phenotypic differences, so it makes sense to focus on them.
...
While we're talking about the transformation, should we standardize the log-expression values?
