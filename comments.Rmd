...
Putting this issue aside, there are several ways of handling dropouts on a per-observation basis:
...
This is particularly true if you imput from similar cells - clustering on the imputed counts would just support the initial similarities.
- Censoring suspected dropouts avoids the need to guess what the non-zero value would be.
However, it's not clear exactly which observations to censor, and if you do it for all of them, it results in loss of a proportion of genuine zeroes.
...
Also, what happens if each cell or gene has a different dropout rate?
...
In fact, it is not clear that there is a need to explicitly model trends in the dropout rates
You can get a natural sigmoidal curve without requiring an explicit zero inflation term in a distribution.
...
With a log-transformation, this is primarily driven by the difference between zero and non-zero values (not so much the size of the non-zero values at low abundances).
...
- As abundances increase further, the variance starts to decrease as the law of large numbers comes into play.
That is, the number of captured molecules per cell will converge to the mean in each cell when you have enough molecules to start with.
- The peak is probably at the abundance where there are comparable numbers of zero and non-zero counts.
