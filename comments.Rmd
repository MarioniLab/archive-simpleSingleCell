...
Leaving the log-expressions unscaled avoids distorting the "true" biological distances (i.e., log-fold changes) between subpopulations.
While any single distance has no meaning, its value is pertinent if you want to accurately represent relative differences between three or more groups of cells.
For example, is one group more related to another, compared to a third group (this is also relevant to subgroups)?
Of course, using unscaled values will not mitigate noise from genes with high technical variance.
However, this is random and not of too much concern - besides, you should have already enriched for biological signal by only using HVGs.
