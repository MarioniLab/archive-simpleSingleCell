...
The critical mean point for `technicalCV2` is dependent on the shape of the technical trend.
The CV^2^ is capped and -- at some point, as the mean decreases -- the trend will pass the cap, preventing detection of genes with lower means.
Here, the choice of mean filter is directly independent of any test against the trend.

Finally, if you consider the "at least _n_" filter, it is possible to get arbitrarily significant HVGs in either method if you pump up the counts in _n-1_ cells.
...
much like how the cell cycle affects the expression of non-cell cycle genes.
...
Using `scLVM` would also work as this would learn the interesting factors of variation and block on them to avoid them getting regressed out with cell cycle phase.
