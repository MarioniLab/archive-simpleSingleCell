...
### Without blocking on known conditions
...
### After blocking on known conditions
...
libsize.drop <- isOutlier(sce$total_counts, nmads=3, type="lower", log=TRUE, batch=blocking)
feature.drop <- isOutlier(sce$total_features, nmads=3, type="lower", log=TRUE, batch=blocking)
spike.drop <- isOutlier(sce$pct_counts_ERCC, nmads=3, type="higher", batch=blocking)
keep <- !(libsize.drop | feature.drop | spike.drop)
data.frame(ByLibSize=sum(libsize.drop), ByFeature=sum(feature.drop),
    BySpike=sum(spike.drop), Remaining=sum(keep))
```
