...
all.urls <- c("https://www.ebi.ac.uk/arrayexpress/files/E-MTAB-5522/E-MTAB-5522.processed.1.zip",
              "https://www.ebi.ac.uk/arrayexpress/files/E-MTAB-5522/E-MTAB-5522.sdrf.txt")
all.basenames <- basename(all.urls)
all.modes <- c("wb", "w")
for (x in seq_along(all.urls)) { 
    download.file(all.urls[x], all.basenames[x], mode=all.modes[x])
}
unzip(all.basenames[1])
```

# Overview

In this article, we use a relatively simple dataset [@lun2017assessing] to introduce most of the concepts of scRNA-seq data analysis.
...
Given the Ensembl identifiers, we can easily obtain the gene symbols using annotation packages like `r Biocpkg("org.Mm.eg.db")`.
...
```{r qcplot416b, fig.wide=TRUE, fig.cap="Histograms of various QC metrics for all cells in the 416B data set. This includes the library sizes, number of expressed genes, and proportion of reads mapped to spike-in transcripts or mitochondrial genes."}
...
```{r qcbiplot416b, fig.wide=TRUE, fig.asp=0.5, fig.cap="Behaviour of each QC metric compared to the total number of expressed features. Each point represents a cell in the 416B dataset."}
par(mfrow=c(1,3))
...
This avoids potential problems with genuine differences in mitochondrial content between cell types that may confound outlier identification.)
...
Figure \@ref(fig:qcplotbatch416b) demonstrates that induction of the CBFB-MYH11 oncogene results in some modest changes to the QC metric distributions.
...
```{r qcplotbatch416b, fig.wide=TRUE, fig.cap="Distribution of each QC metric for control and oncogene-induced cells in the 416B data set."}
multiplot(
...
We use the Ensembl identifiers for each gene in our data set to match up with the names in the pre-trained set of gene pairs.
...
```{r topgene416b, fig.asp=1.5, fig.wide=TRUE, fig.cap="Percentage of total counts assigned to the top 50 most highly-abundant features in the 416B dataset. For each feature, each bar represents the percentage assigned to that feature for a single cell, while the circle represents the average across all cells. Bars are coloured by the total number of expressed features in each cell, while circles are coloured according to whether the feature is labelled as a control feature."}
...
```{r nexprshist416b, fig.cap="The number of cells expressing each gene in the 416B data set, plotted against the log-average count. Intensity of colour corresponds to the number of genes at any given location."}
...
Finally, users can set `use.spikes=FALSE` to fit a trend to the variances of the endogenous genes in the absence of spike-in transcripts.
This requires some care in interpretation of the results, which will be discussed later.
...
As previously mentioned, the use of `design` in `trendVar` will ensure that systematic differences between plates do not inflate the variance.
...
We demonstrate this approach by treating each plate in the 416B dataset as a different batch (Figure \@(fig:hvgplotbatch416b)).

```{r hvgplotbatch416b, fig.wide=TRUE, fig.asp=0.5, fig.cap="Variance of normalized log-expression values for each gene in each plate of the 416B dataset, plotted against the mean log-expression. The blue line represents the mean-dependent trend fitted to the variances of the spike-in transcripts (red)."}
collected <- list()
par(mfrow=c(1,2))
for (plate in levels(sce$Plate)) {
    cur.sce <- sce[,sce$Plate==plate]
...
These values can be used in downstream functions in place of the corresponding values from `var.out`.
...
- When `design` is specified, `denoisePCA` will automatically regress out uninteresting factors of variation and perform PCA on the residuals.
This ensures that those factors do not drive separation between clusters or in low-dimensional space.
Users can do this manually with the `removeBatchEffect` function from the `r Biocpkg("limma")` package [@ritchie2015limma], as shown below.
...
adj.exprs <- removeBatchEffect(adj.exprs, batch=sce$Plate)
...
We visualize the relationships between cells by constructing pairwise PCA plots for the first three components (Figure \@ref(fig:pcaplot416b)).
...
```{r pcaplot416b, fig.cap="Pairwise PCA plots of the first three PCs in the 416B data set, constructed from normalized log-expression values of genes with positive biological components. Each point represents a cell, coloured according to the oncogene induction status. Bars represent the coordinates of the cells on each axis.", fig.width=9}
plotReducedDim(sce, use_dimred="PCA", ncomponents=3, colour_by="Oncogene") + fontsize
```
...
```{r tsneplot416b, fig.cap="_t_-SNE plots constructed from the denoised PCs in the 416B data set, using a range of perplexity values. Each point represents a cell, coloured according to its oncogene induction status. Bars represent the coordinates of the cells on each axis.", fig.width=15, fig.asp=0.5}
out5 <- plotTSNE(sce, use_dimred="PCA", perplexity=5, colour_by="Oncogene", 
    rand_seed=100) + fontsize + ggtitle("Perplexity = 5")
out10 <- plotTSNE(sce, use_dimred="PCA", perplexity=10, colour_by="Oncogene",
    rand_seed=100) + fontsize + ggtitle("Perplexity = 10")
out20 <- plotTSNE(sce, use_dimred="PCA", perplexity=20, colour_by="Oncogene",
...
```{r tsnecluster416b, fig.cap="_t_-SNE plot of the denoised PCs of the 416B data set. Each point represents a cell and is coloured according to the cluster identity to which it was assigned."}
...
The aim is to test for DE in each cluster compared to the others while blocking on uninteresting factors such as the plate of origin in `design`.
...
A cursory examination of the DE genes suggests that cluster 1 contains oncogene-induced cells with no expression of cell cycle genes.
This is consistent with senescence as a tumour suppression mechanism.

```{r heatmapmarker416b, fig.wide=TRUE, fig.cap="Heatmap of mean-centred and normalized log-expression values for the top set of markers for cluster 1 in the 416B dataset. Column colours represent the cluster to which each cell is assigned, the plate of origin or the oncogene induction status of each cell, as indicated by the legend."}
...
colnames(heat.vals) <- paste0("Cell", seq_len(ncol(heat.vals)))
...
library(pheatmap)
pheatmap(heat.vals, cluster_cols=my.tree, breaks=col.breaks, 
    annotation_col=data.frame(Cluster=factor(my.clusters), Plate=factor(sce$Plate),
        Oncogene=sce$Oncogene, row.names=colnames(heat.vals)),
    annotation_colors=list(Cluster=setNames(chosen.cols, unique.clusters),
        Plate=setNames(c("grey50", "grey80"), levels(sce$Plate)),
        Oncogene=setNames(c("red", "black"), levels(sce$Oncogene))))
```
