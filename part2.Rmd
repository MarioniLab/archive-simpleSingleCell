...
Nonetheless, we compute some quality control metrics with `r Biocpkg("scater")` [@mccarthy2016scater] to check whether the remaining cells are satisfactory.
...
- `calcAverage` returns a library size-adjusted average count, which is proportional to the average of the values used internally by `computeSumFactors`.
While not entirely independent [@bourgon2010independent], this is a better filter statistic than the average count without adjustment for library size.
