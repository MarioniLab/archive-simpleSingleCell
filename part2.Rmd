...
To assess clustering performance, we calculate the total weight of edges between cells in the same cluster.
We divide this by the total weight of edges involving a cell in that cluster to obtain a per-cluster modularity score.
High modularity scores (near the maximum of 1) should be observed if a cell is mostly connected to other cells in the same cluster compared to cells in other clusters.

```{r}
grmat <- snn.gr[]
total.weight <- Matrix::colSums(grmat)/2 # Symmetric for undirected graphs
mod.cluster <- numeric(nlevels(sce$cluster))
names(mod.cluster) <- levels(sce$cluster)
for (x in names(mod.cluster)) {
    current <- sce$cluster==x
    internal.weight <- sum(grmat[current,current])/2
    mod.cluster[x] <- internal.weight/sum(total.weight[current])
}
mod.cluster
```
...
- Notice that we do not run `library(igraph)`. 
...
We use the `findMarkers` function to identify marker genes for each cluster, as previously described.
We examine the output table for cluster 1, in which log-fold changes are reported between cluster 1 and every other cluster.
...
```{r heatmapmarkerbrain, fig.wide=TRUE, fig.asp=1.5, fig.cap="Heatmap of mean-centred and normalized log-expression values for the top set of markers for cluster 1 in the brain dataset. Column colours represent the cluster to which each cell is assigned, as indicated by the legend."}
...
clust.col <- clust.col[seq_along(unique.clust)]

library(pheatmap)
pheatmap(heat.vals[,order(my.clusters)], cluster_cols=FALSE,
    annotation_col=data.frame(Cluster=factor(my.clusters), row.names=colnames(sce)),
    annotation_colors=list(Cluster=setNames(clust.col, unique.clust)))
```
