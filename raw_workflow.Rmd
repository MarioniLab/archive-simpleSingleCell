...
This article describes a computational workflow for basic analysis of scRNA-seq data, using software packages from version 3.4 of the open-source Bioconductor project [@huber2015orchestrating].
...
In contrast, calculating correlations for all possible gene pairs would require too much computational time and increase the severity of the multiple testing correction, and may prioritize uninteresting genes that have strong correlations but low variance, e.g., tightly co-regulated house-keeping genes.
...
However, a small number of cells have ambiguous classifications, lying in the top-right of the plot with high scores for both G1 and G2/M phases.
This uncertainty may be caused by differences between the test and training data.
...
These new biases (and the absence of expected biases) may interfere with accurate classification of these cells.
...
An additional complication is that many neuronal cell types are quiescent and do not belong in any phase of the cell cycle [@coller2006new].
It is not clear how these cells should be classified -- so, for simplicity, we will not perform any processing of this dataset by cell cycle phase.
...
Versions for all Bioconductor packages correspond to release version 3.4 of the Bioconductor project.
