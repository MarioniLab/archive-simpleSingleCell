...
These genes are likely to be dominated by drop-out events [@brennecke2013technical] which reduces their usefulness for downstream analyses.
...
To check whether the chosen threshold is suitable, we can examine the distribution of log-means across all genes (Figure ((abhisthsc))).
...
These visual subpopulations are computationally defined by performing a tree cut to identify explicit clusters.
...
This allows genes to be retained at lower abundances as the biological signal is no longer dominated by noise.
