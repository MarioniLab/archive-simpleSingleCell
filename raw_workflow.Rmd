...
The size factors are log-transformed and used throughout as GLM offsets to normalize cell-specific biases.
...
We perform some filtering to retain only endogenous genes that are present in moderate to high abundance.
Note that quality control on the cells is not required as it has already been performed by @kolod2015singlecell. 

```{r}
keep <- grepl("^ENSMUS", rownames(sce)) & rowMeans(counts(sce)) >= 1
sce <- sce[keep,]
dim(sce)
```
...
Size factors are log-transformed to use as offsets during GLM fitting in the `r Biocpkg("edgeR")` pipeline.
We also add some gene symbol annotation for easier interpretation of the results.

```{r}
y <- DGEList(counts(sce))
y$offset <- log(sizeFactors(sce)) 
y$genes <- fData(sce)[,"SYMBOL",drop=FALSE]
```
...
The rest of the analysis can proceed using any bulk analysis pipeline.
Standard normalization methods can be applied as the counts are large enough to avoid problems with zeros.
The NB dispersion is estimated and a GLM is fitted to the counts using the specified design matrix.
We also estimate the quasi-likelihood (QL) dispersion to account for the uncertainty in dispersion estimation [@lund2012detecting].
Significant differences between the 2i and a2i conditions are identified using the QL F-test. 

```{r}
y2 <- DGEList(summed)
y2$genes <- fData(sce)[,"SYMBOL",drop=FALSE]
y2 <- calcNormFactors(y2)
y2 <- estimateDisp(y2, design2, robust=TRUE)
fit2 <- glmQLFit(y2, design2, robust=TRUE)
res2 <- glmQLFTest(fit2, contrast=makeContrasts(culture22i-culture2a2i, levels=design2))
topTags(res2)
```
...
It should be stressed that DE detection does not require a shift in the entire distribution of expression values for all cells in one condition.
An increase in expression in the upper tail of the distribution is also relevant and will be detected, provided there are sufficient cells in that tail.

```{r hvgboxplotmescbulk, fit.width=10, fig.height=6, fig.cap="Boxplots of batch-corrected normalized log-expression values for the top 10 DEGs in the mESC data set, detected by QL methods after summing counts for all cells on each plate."}
