...
```{r}
culture2 <- sub("\\..*$", "", colnames(summed))
batch2 <- sub("^.*\\.", "", colnames(summed))
design2 <- model.matrix(~0 + culture2 + batch2)
colnames(design2)
```
...
```{r}
y2 <- DGEList(summed, genes=fData(sce)[,"SYMBOL",drop=FALSE])
y2 <- y2[!isSpike(sce),]
y2 <- calcNormFactors(y2)
y2 <- estimateDisp(y2, design2, robust=TRUE)
fit2 <- glmFit(y2, design2)
res2 <- glmLRT(fit2, contrast=makeContrasts(culture22i-culture2a2i, levels=design2))
topTags(res2)
```
