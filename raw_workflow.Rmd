...
However, for the same reason, detection power is reduced for HVGs driven by rare subpopulations.
If these rare cells are of interest, alternative methods for HVG identification may be preferable, e.g., using the `DM` method in `r Biocpkg("scran")`.
...
<!--
An alternative might be to compute the significance of each cluster to the average of all other clusters.
...
Unlike in the previous analyses, we do not have to additionally set spike-in-specific size factors, as these are already being used for all genes and spike-in transcripts.
