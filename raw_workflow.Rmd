...
rm(list=setdiff(ls(), c("all.basenames", "on.bioc")))
gc()
```
...
Rather, a more sophisticated strategy is required, which is demonstrated below using data from a study of T Helper 2 (Th2) cells [@mahata2014singlecell].
...
We block on the (empirical) cell cycle phases in the design matrix prior to downstream analyses.
...
For downstream steps that do _not_ use a design matrix, we can use `removeBatchEffect` to remove the effect of cell cycle phase on the gene expression profiles.
...
```{r pcaplotth2, fig.width=16, fig.height=8, fig.cap="PCA plots before (left) and after (right) removal of the cell cycle effect in the Th2 data set. Each point represents a cell, coloured according to the empirically assigned cell cycle phase (G2/M and S phases were pooled prior to coloration, due to the low numbers of cells in each phase). Only the top 500 HVGs were used to make each PCA plot."}
fit <- trendVar(sce, use.spikes=NA, trend="loess")
dec <- decomposeVar(sce, fit)
top.hvgs <- order(dec$bio, decreasing=TRUE)[1:500]
pr.out <- prcomp(t(exprs(sce)[top.hvgs,]), scale=TRUE)

corrected <- removeBatchEffect(exprs(sce), batch=hard.assign)
top.hvgs2 <- order(dec.block$bio, decreasing=TRUE)[1:500]
pr.out2 <- prcomp(t(corrected[top.hvgs2,]), scale=TRUE)

par(mfrow=c(1,2))
base.colors <- c(G1="grey50", "not G1"="red")
colors <- base.colors[ifelse(hard.assign=="G1", 1, 2)]
plot(pr.out$x[,1], pr.out$x[,2], col=colors, pch=16, xlab="PC1", ylab="PC2", main="Before removal")
legend("bottomleft", legend=names(base.colors), col=base.colors, pch=16)
plot(pr.out2$x[,1], pr.out2$x[,2], col=colors, pch=16, xlab="PC1", ylab="PC2", main="After removal")
```

Note that, if an analysis can accept a design matrix, then blocking on nuisance factors in the design matrix is preferable to manipulating the expression values with `removeBatchEffect`.
...
As an aside, this data set contains Th2 cells in various states of differentiation [@mahata2014singlecell].
This is an ideal use case for diffusion maps which perform dimensionality reduction along a continuous process.
In Figure ((diffusionth2)), cells are arranged along a trajectory corresponding to increasing levels of differentiation.
This is evident from the left-to-right change in the expression levels of _Gata3_, which is a gene involved in Th2 differentiation.

```{r diffusionth2, fig.cap="A diffusion map for the Th2 data set, where each cell is coloured by its expression of _Gata3_."}
plotDiffusionMap(sce, colour_by="Gata3") + fontsize
```
