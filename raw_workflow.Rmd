...
This article describes a computational workflow for basic analysis of scRNA-seq data, using software packages from the open-source Bioconductor project (release 3.4) [@huber2015orchestrating].
...
<!--
In general, it seems to more sense to apply cyclone before filtering out low-abundance genes.
...
We don't bother imputing, because we don't know whether a zero is a true dropout or a genuine zero.
...
<!--
As opposed to technical noise, which should be random and reduce the correlations (and power). 

An alternative approach would be a method that detects correlations and HVGs at the same time, where strong correlations would offset low variances for gene detection.
...
<!--
Misclassification due to the differences in training/test datasets, which may lead to cells being assigned to phases that they're not; 
or misclassification due to G0, which should be closest to G1 but may be different enough that you'd get a low G1 score => more S-like?
-->
...
Here, the proportion of significantly correlated pairs is much higher than in the HSC dataset, indicating that strong substructure is present.
...
These pairs are saved to file for future use in designing validation experiments.

```{r}
cor.out <- gzfile("brain_cor.tsv.gz", open="wb") # Compression to save space.
write.table(file=cor.out, var.cor, sep="\t", quote=FALSE, row.names=FALSE)
close(cor.out)
```
...
<!--
It may be a more relevant set, because you can detect HVGs at greater power if you didn't have uninvolved, constantly-expressing cells dragging down the variance/correlations.
...
Version numbers of all Bioconductor packages correspond to release version 3.4 of the Bioconductor project.
