...
This is done by applying methods from the `r Biocpkg("limma")` package to the log-expression values.
...
Other trends can be more flexibly modelled using splines, i.e., `~splines::ns(pc1, df=5)`.
However, users should keep in mind that the spline coefficients are not easily interpretable.
