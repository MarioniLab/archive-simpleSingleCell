...
author: 
    ...
    - name: Davis J. McCarthy
      affiliation: EMBL European Bioinformatics Institute, Wellcome Genome Campus, Hinxton, Cambridge CB10 1SD, United Kingdom
    - name: John C. Marioni
...
"http://linnarssonlab.org/blobs/cortex/expression_mRNA_17-Aug-2014.txt", 
"http://linnarssonlab.org/blobs/cortex/expression_mito_17-Aug-2014.txt", 
"http://linnarssonlab.org/blobs/cortex/expression_spikes_17-Aug-2014.txt",
...
This is necessary as spike-ins require special treatment in some downstream steps (e.g., normalization, see below).
We can do this easily by indicating the name of the spike-in set to `isSpike`.
...
Here, low-abundance genes are defined as those with an average count below 1.
This threshold is chosen as genes with lower averages are more likely to be dominated by zero counts (based roughly on a negative binomial model for technical noise in the counts).
Removing such genes mitigates discreteness and reduces the amount of computational work without major loss of information.

<!--
Simulations indicate that number of zeros from a NB with mean of 1 and dispersion of 1 is ~50%, e.g., sum(rnbinom(1000, mu=1, size=1)==0)
-->
...
We can also have a look at the identities of the top set of retained high-abundance genes.
...
```{r}
sort(ave.counts, decreasing=TRUE)[1:50]
```
...
```{r}
alt.keep <- nexprs(sce, byrow=TRUE) >= 10
sum(alt.keep)
```
...
This assumes that all spike-in transcripts are not differentially expressed -- which is a reasonable assumption, as the same spike-ins should have been added to each cell.
...
This is not unexpected for a homogenous population of cells of the same type.
...
As such, determining whether a cluster is "real" or not usually depends on subjective judgement, unless the clusters are very clearly defined (i.e., strong separation on a PCA plot, widespread differences in the expression profiles).
...
(The technical noise is also lower, which reduces the risk of zeroes dominating the expression profile and allows us to retain genes with lower means.)
Even so, the number of retained genes is lower than that in the HSC data set, simply because the library sizes are much smaller for UMI counts.

<!--
sum(rpois(1000, lambda=0.5)==0) # around 50-60%.
Also interpretable as you need at least expression in half of your cells; genes that are lost won't even have that.
-->

```{r}
keep <- rowMeans(counts(sce)) >= 0.5
sce <- sce[keep,]
sum(keep)
```
...
Nonetheless, some neuron types may be problematic for the classifier as they are postmitotic and do not belong in any phase of the cell cycle.
...
This is likely to be a relevant biological effect rather than a technical one, e.g., different subpopulations in different tissues.
...
Strictly speaking, this is unnecessary as there are no differences between sexes in Figure ((pcaplotbrain)).
...
This is done by testing for DE between each pair of subpopulations and selecting those genes that are consistently upregulated in one subpopulation compared to all others.
...
This is done using the likelihood ratio test (LRT) for each comparison, as demonstrated below for cluster 1.
...
Potential marker genes for cluster 1 are ranked based on the maximum _p_-value across all comparisons.
...
This is equivalent to the outcome of an intersection-union test [@berger1996bioequivalence] requiring all comparisons to other clusters to be significant.
In addition, we only focus on genes with a positive log-fold change for all comparisons.
This is necessary to identify specific markers that are unambiguously upregulated in cluster 1 relative to the other clusters.

```{r, results="hide"}
max.PValue <- do.call(pmax, result.PValue)
all.logFC <- do.call(cbind, result.logFC)
same.sign <- rowSums(all.logFC > 0)==ncol(all.logFC)
marker.set <- data.frame(Gene=rownames(y), logFC=all.logFC, PValue=max.PValue, 
    FDR=p.adjust(max.PValue, method="BH"), stringsAsFactors=FALSE)
...
<!--
Correcting before filtering, as the filter isn't independent for correlated log-fold changes.
-->
...
Figure ((heatmapmarkerbrain)) indicates that all of the top markers have strong and consistent upregulation in cells of cluster 1 compared to every other cluster.
If this is not the case, it suggests there are few unique markers for the corresponding subpopulation.

```{r heatmapmarkerbrain, fig.cap="Heatmap of mean-centred normalized log-expression values for the top set of markers for cluster 1 in the brain data set. Column colours represent the cluster to which each cell is assigned."}
write.table(marker.set, file="brain_marker_1.tsv", sep="\t", quote=FALSE, row.names=FALSE)
...
<!--
Yet another approach might be to compute the significance of each cluster to the average of all other clusters.
This is also less stringent than the marker definitions above, as it allows for a gene to be expressed in another cluster.
However, it's also a bit unpredictable as a gene may or may not be detected depending on the level of expression in other (potentially unrelated) populations.
-->
...
any.de <- data.frame(Gene=rownames(y), log.fold.changes, PValue=res.any$table$PValue, 
    FDR=p.adjust(res.any$table$PValue, method="BH"), stringsAsFactors=FALSE)
...
For such analyses, the FDR-corrected _p_-value can be directly used to define significant genes for each DE comparison, though some care may be required to deal with plate effects [@hicks2015widespread; @tung2016batch; @lun2016overcoming].
...
(The exception is if there are multiple spike-in sets used in the experiment, that behave differently and need to be normalized separately.)
