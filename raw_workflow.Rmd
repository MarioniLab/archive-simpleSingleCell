...
Thus, it is common for low-abundance genes to be removed at the start of many RNA-seq analysis pipelines.

Several metrics can be used to define low-abundance genes.
We compute the average count across all cells in the HSC data set, with some adjustment for library size differences between cells (Figure \@ref(fig:abhisthsc)).
...
Alternatively, we could remove genes that are expressed in fewer than _n_ cells.
...
Here, we save the average count into the `SCESet` object for future use.

```{r}
fData(sce)$ave.counts <- ave.counts
```

It is important to note that the "optimal" choice of filtering strategy depends on the downstream application.
...
For testing procedures, the filter statistic should also be independent of the test statistic under the null hypothesis [@bourgon2010independent].
Thus, we will filter at each step as needed, rather than applying a single filter for the entire analysis.
We only remove genes that are not expressed in any cell, as such genes provide no information and would be removed by any filtering strategy.
...
A threshold of 1 is usually satisfactory for read count data.
...
The default threshold was chosen by observing the appearance of discrete patterns in the variance estimates for Poisson-distributed counts.
However, the threshold can also be manually specified by setting `mean.threshold`.
...
For example, we might filter out low-abundance genes by setting a minimum threshold of 0.1 on the average UMI count.
