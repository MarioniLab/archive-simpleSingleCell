...
examined <- rownames(hvg.out)[1:10]
all.names <- matrix(examined, nrow=length(examined), ncol=ncol(sce))
boxplot(split(exprs(sce)[examined,], all.names), las=2, 
    ylab="Normalized log-expression", col="grey80")
```
...
Specifically, the use of UMIs eliminates technical noise due to amplification biases.
