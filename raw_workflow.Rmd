---
title: "Cell by cell, count to count: a step-by-step workflow for basic analyses of single-cell RNA-seq data"
author: 
    - name: Aaron T. L. Lun
...
One measure of quality is the library size, defined as the total sum of counts across all genes and spike-in transcripts.
...
In general, any cell with a library size that is an order of magnitude lower than the median across all cells should be removed.
A more relaxed threshold may be necessary when strong heterogeneity in total RNA content is expected across cells.

<!-- 
Easier to interpret than using outliers here, as we'd need to log the library sizes before MAD'ing them.
-->

```{r}
lib.sizes <- sce$total_counts
summary(lib.sizes)
keep <- lib.sizes >= median(lib.sizes)/10
sce <- sce[,keep]
sum(!keep)
```

The number of genes expressed in each cell is calculated as the number of non-zero counts.
Any cell with very few expressed genes is considered to be of poor quality as the diverse transcript population has not been successfully captured.
We remove cells with fewer than 1000 expressed genes to ensure that the total count in each cell is not driven by a small number of transcripts.
...
Here, we define outliers as those with proportions that are more than 3 median absolute deviations (MADs) above the median proportion for spike-in transcripts or mitochondrial genes.
...
In Figure ((pcaqualplothsc)), no cells were identified as outliers which is consistent with the removal of suspect cells in the preceding quality control steps.
...
Users interested in the more sophisticated approaches are advised to refer to the `r Biocpkg("scater")` and `r Biocpkg("cellity")` packages.
...
The relationship between the proportion of expressing cells and the mean can be examined more closely in Figure ((geneplothsc)).
...
In this case, the size factors are tightly correlated with the total counts for all cells (Figure ((normplothsc))).
...
The log-transformation is useful as it provides some variance stabilization -- this ensures that high-abundance genes with large variances do not dominate downstream analyses.
...
This can be visualized by constructing a PCA plot from the normalized log-expression values (Figure ((pcaplothsc))).
Cells with similar expression profiles should be located close together in the plot.
...
Alternatively, the PCA plot can be constructed from a matrix of correlations between cells (Figure ((pcacorplothsc))). 
...
_t_-SNE tends to work better than PCA for large data sets with many subpopulations, at the cost of more computational effort and complexity.
...
These have their own advantages and disadvantages -- for example, diffusion maps are particularly suited to mapping cells along some continuous process like differentiation.
...
In contrast, the spike-in trend is less stable at mean intervals with few spike-in transcripts.
...
Of course, there are many ways of defining HVGs, e.g., by using the coefficient of variation [@kolod2015singlecell;@kim2015characterizing], with the dispersion parameter in the negative binomial distribution [@mccarthy2012differential], or as a proportion of total variability [@vallejos2015basics].
...
This can be verified by directly using sets of correlated HVGs as markers in procedures such as fluorescence-activated cell sorting, immunohistochemistry or RNA flourescence _in situ_ hybridization.
...
In the simplest example, a subpopulation is positive for one marker in the pair and negative for the other marker, while the reverse is true for a different subpopulation.
...
We can visualize the constructed dendrogram with a heatmap (Figure ((heatmaphsc))).
...
For example, cells can be ordered by pseudotime (e.g., for differentiation) with `r Biocpkg("monocle")` [@trapnell2014dynamics]; cell-state hierarchies can be characterized with the `r Biocpkg("sincell")` package [@julia2015sincell]; and oscillatory behaviour can be identified using `r Biocpkg("Oscope")` [@leng2015oscope].
...
```{r}
lib.sizes <- sce$total_counts 
count.keep <- lib.sizes >= median(lib.sizes)/10
sum(!count.keep)
...
Compared to the HSC analysis, more scatter is observed for the trend between the total count and size factor for each cell (Figure ((normplotbrain))).
...
This strategy reduces the complexity of the colours in each plot and improves the ease of diagnosis if systematic differences between chips or mice are present.
...
The heatmap in Figure ((heatmapbrain)) shows systematic differences between groups of cells with distinct patterns of expression.
...
The size factors are log-transformed and used as normalization offsets in the generalized linear models (GLMs).
...
The heatmap in Figure ((heatmapmarkerbrain)) indicates that all of the top markers are strongly expressed across most cells in cluster 2, whereas expression is lower or absent in the other clusters.
...
```{r heatmapmarkerbrain, fig.cap="Heatmap of mean-centred normalized log-expression values for the top set of markers for cluster 2 in the brain data set. Column colours represent the cluster to which each cell is assigned."}
...
Indeed, Figure ((normplotspikehsc)) shows a rough positive correlation between the two sets of size factors, consistent with removal of technical biases by both methods.
...
The majority of cells in Figure ((phaseplotth2)) are assigned to the G1 or G2/M phases, with small numbers of cells in S-phase or without clear assignments.
...
We use `removeBatchEffect` to remove the effect of the (empirical) cell cycle phase on the gene expression profiles.
The result of this procedure can be visualized with some PCA plots in Figure ((pcaplotth2)).
...
```{r pcaplotth2, fig.width=16, fig.height=8, fig.cap="PCA plots before (left) and after (right) removal of the cell cycle effect in the Th2 data set. Each point represents a cell, coloured according to the empirically assigned cell cycle phase. Only the top 500 HVGs were used to make the PCA plot before removal. Cells were projected onto the same PCA space after removal to facilitate comparisons between plots."}
fit <- trendVar(sce, use.spikes=NA)
top.hvgs <- order(fit$var, decreasing=TRUE)[1:500]
pr.out <- prcomp(t(exprs(sce)[top.hvgs,]))
corrected <- removeBatchEffect(exprs(sce), batch=hard.assign)
pr.out2 <- predict(pr.out, t(corrected[top.hvgs,]))

par(mfrow=c(1,2))
xrange <- range(c(pr.out$x[,1], pr.out2[,1]))
yrange <- range(c(pr.out$x[,2], pr.out2[,2]))
colors <- c(G1="grey50", S="orange", G2M="red", unknown="dodgerblue")
plot(pr.out$x[,1], pr.out$x[,2], col=colors[hard.assign], pch=16, xlab="PC1", ylab="PC2", 
    main="Before removal", xlim=xrange, ylim=yrange)
legend("bottomleft", legend=names(colors), col=colors, pch=16)
plot(pr.out2[,1], pr.out2[,2], col=colors[hard.assign], pch=16, xlab="PC1", ylab="PC2", 
    main="After removal", xlim=xrange, ylim=yrange)
```
...
However, they are difficult to interpret compared to commonly used gene symbols.
...
### Using the single-cell counts

Some data sets contain pre-defined biological groups in the experimental design, e.g., tissue and sex in the brain data set.
If differences between these groups are of interest, a DE analysis can be performed using the cells in each group.
This is demonstrated with the mESC data set which contains several batches of one or more C1 chips.
Each chip contains cells from only one culture condition, with three conditions in total across the entire experiment.
We set up the design matrix, using an additive design to account for the different culture conditions and the batch effect.

```{r}
culture <- sub("^ola_mES_([^_]+)_.*", "\\1", colnames(sce))
batch <- sub("^ola_mES_[^_]+_([^_]+)_.*", "\\1", colnames(sce))
design <- model.matrix(~0 + culture + batch)
colnames(design)
```

We perform some filtering to retain only genes that are present in moderate to high abundance.
Note that quality control on the cells is not required as it has already been performed by @kolod2015singlecell. 
...
To normalize cell-specific biases, we use the deconvolution approach to compute size factors.
We assume that most genes are not DE, which seems reasonable given that all cells are of the same type.

```{r}
cluster <- quickCluster(sce)
sce <- computeSumFactors(sce, cluster=cluster)
summary(sizeFactors(sce))
```

We construct a `DGEList` to prepare for a DE analysis with `r Biocpkg("edgeR")`.
We add some gene symbol annotation to simplify the interpretation of the results.
Spike-in transcripts are removed as differences in spike-in coverage are neither expected or interesting.
Size factors are also log-transformed to use as offsets during GLM fitting.

```{r}
y <- DGEList(counts(sce), genes=fData(sce)[,"SYMBOL",drop=FALSE])
y <- y[!isSpike(sce),]
y$offset <- log(sizeFactors(sce)) 
```

We then test for DEGs between the ground state culture (2i) and the alternative ground state (a2i) using the LRT.
This process is similar to the identification of marker genes for the brain data set, except that pre-defined variables are used in the design matrix instead of empirical clusters.
Keep in mind that dispersion estimation may take some time for large data sets.

```{r}
y <- estimateDisp(y, design, robust=TRUE)
fit <- glmFit(y, design)
res <- glmLRT(fit, contrast=makeContrasts(culture2i-culturea2i, levels=design))
topTags(res)
```

We examine the top DEGs with boxplots of batch-corrected log-expression values (Figure ((hvgboxplotmesc)).
Robust differences in expression are generally observed between the culture conditions, indicating that the results are not driven by outlier cells.

```{r hvgboxplotmesc, fit.width=10, fig.height=6, fig.cap="Boxplots of batch-corrected normalized log-expression values for the top 10 DEGs in the mESC data set. Points correspond to cells that are more than 1.5 interquartile ranges from the edge of each box."}
sce <- normalize(sce)
debatched <- removeBatchEffect(exprs(sce), batch=batch)
is.tested <- culture %in% c("2i", "a2i")
examined <- order(res$table$PValue)[1:10]
cur.genes <- fData(sce)$SYMBOL[examined]
all.names <- outer(cur.genes, culture[is.tested], FUN=function(x, y) paste0(x, " (", y, ")"))
par(mar=c(7.1, 4.1, 1.1, 1.1))
boxplot(split(debatched[examined,is.tested], all.names), ylab="Normalized log-expression",
    las=2, col=rep(rep(c("grey80", "lightblue"), each=2), length(examined)/2))
```

As the groups of cells are pre-defined, issues with clustering errors are avoided.
This means that the DE _p_-values can be directly interpreted as measures of significance.
For example, the number of DEGs in each direction at a FDR threshold of 5% can be identified.
...
### Using pseudo-bulk counts after summation

An alternative approach is to sum the counts for all cells within each plate/chip.
Each plate is equivalent to a sample in a bulk RNA-seq experiment, with plates in the same group becoming replicate samples.
Processing of this "pseudo-bulk" counts can then be performed to detect DEGs between groups.
This approach avoids problems from plate effects that can invalidate analyses based on single-cell counts [@lun2015overcoming].
It also increases the size and precision of the counts, making them more amenable for analysis with methods designed for bulk data.

This strategy can be demonstrated using the mESC data set.
Cells from the same plate are identified and their counts are summed together with the `sumTechReps` function.
In this case, only one C1 chip is used for each culture in each batch.
Identification can thus be performed based on the combination of the culture/batch factors defined for each cell.

```{r}
plate.of.origin <- paste0(culture, ".", batch)
summed <- sumTechReps(counts(sce), plate.of.origin)
head(summed)
```

We set up the design matrix using the culture and batch factors for each plate.
Each plate is effectively treated as an independent sample within this design.

```{r}
culture.sum <- sub("\\..*$", "", colnames(summed))
batch.sum <- sub("^.*\\.", "", colnames(summed))
design.sum <- model.matrix(~0 + culture.sum + batch.sum)
colnames(design.sum)
```

The rest of the DE analysis can proceed using any bulk-based pipeline such as `r Biocpkg("edgeR")`.
Normalization between plates can be performed using standard methods like `calcNormFactors`, as the counts are large enough to avoid problems with zeros.
For each gene, the NB dispersion is estimated and a GLM is fitted to the counts using the specified design matrix.
Significant differences between the 2i and a2i conditions are identified using the LRT.

```{r}
y.sum <- DGEList(summed, genes=fData(sce)[,"SYMBOL",drop=FALSE])
y.sum <- y.sum[!isSpike(sce),]
y.sum <- calcNormFactors(y.sum)
y.sum <- estimateDisp(y.sum, design.sum, robust=TRUE)
fit.sum <- glmFit(y.sum, design.sum)
res.sum <- glmLRT(fit.sum, contrast=makeContrasts(culture.sum2i-culture.suma2i, levels=design.sum))
topTags(res.sum)
```

The boxplots in Figure ((hvgboxplotmescbulk)) suggest that each of the top genes is exhibiting robust DE across a number of cells.
It should be stressed that the concept of DE is not limited to a global shift in the distribution of expression values for all cells in one group.
An increase in expression across the upper tail of the distribution will also be detected as DE, provided there are sufficient cells in that tail.
This may be relevant, e.g., if a gene is only upregulated in a particular subpopulation of one group.

```{r hvgboxplotmescbulk, fit.width=10, fig.height=6, fig.cap="Boxplots of batch-corrected normalized log-expression values for the top 10 DEGs in the mESC data set, detected by after summing counts for all cells on each plate."}
examined <- order(res2$table$PValue)[1:10]
cur.genes <- fData(sce)$SYMBOL[examined]
all.names <- outer(cur.genes, culture[is.tested], FUN=function(x, y) paste0(x, " (", y, ")"))
par(mar=c(7.1, 4.1, 1.1, 1.1))
boxplot(split(debatched[examined,is.tested], all.names), ylab="Normalized log-expression",
...
Several hundred DEGs are detected between the two culture conditions at a FDR of 5%.
In general, the summation approach will yield fewer DEGs compared to using the single-cell counts directly.
This is because type I error control is lost in the latter when the analysis is compromised by plate effects [@lun2015overcoming].
...
In addition, the processed data can be easily used for higher-level analyses such as pseudotime ordering with `r Biocpkg("monocle")`.
...
This work was supported by CRUK core funding (SW73) to J.C.M.
