...
```{r, warning=FALSE}
high.ab <- fData(sce)$ave.counts >= 1
...
We define the filter vector `high.ab` with a threshold of 1 on the average count, which is usually satisfactory for read count data.
...
var.fit <- trendVar(sce, parametric=TRUE, subset.row=high.ab, span=0.2)
```
...
The threshold of 0.1 was chosen (`high.ab`) by observing the appearance of discrete patterns in the variance estimates for simulated Poisson-distributed counts.
...
Note that this variance decomposition is performed for all genes in `sce`.
...
sce <- computeSumFactors(sce, cluster=clusters, subset.row=high.ab, mean.warn=FALSE)
summary(sizeFactors(sce))
```

We set `subset.row` again to use only the high-abundance genes for normalization (and clustering, for consistency).
As previously mentioned, we use a threshold of 0.1 to define `high.ab`. 
...
For trend fitting, we filter on the average log-expression for each gene (`high.ab`) to avoid problems with discreteness.
...
var.fit <- trendVar(sce, parametric=TRUE, span=0.4, 
    subset.row=high.ab, design=design)
var.out <- decomposeVar(sce, var.fit)
```
...
for (block in levels(sce$sex)) {
    ...
    high.ab <- rowMeans(exprs(cur.sce)) >= 0.1
    var.fit <- trendVar(cur.sce, subset.row=high.ab, 
...
Here, we filter out low-abundance genes in both `trendVar` _and_ `decomposeVar` by setting `subset.row=high.ab`.
This reflects our focus on high-abundance genes with large variances that are the largest contributors to population heterogeneity.
Filtering reduces the severity of the multiple testing correction, and also avoids problems with computing _p_-values for discrete variance estimates.

```{r}
sce <- readRDS("hsc_data.rds")
high.ab <- rowMeans(exprs(sce)) > 0.1
var.fit <- trendVar(sce, subset.row=high.ab, parametric=TRUE, span=0.2)
var.out <- decomposeVar(sce, var.fit, subset.row=high.ab)
