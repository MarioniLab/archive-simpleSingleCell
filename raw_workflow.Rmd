...
Once putative subpopulations are identified by clustering, we can identify some candidate marker genes that distinguish between those subpopulations.
This is done by testing for DE between pairs of subpopulations and selecting those genes that are consistently upregulated in one subpopulation compared to the others.
...
We identify marker genes for a chosen subpopulation as those that are upregulated in the corresponding cluster.
This is done by first testing each gene for DE between the chosen cluster and every other cluster.
We demonstrate this for cluster 1, though the same process can be applied to every other cluster by changing `chosen.clust`.
...
Potential marker genes are identified by taking the top set of upregulated genes from each pairwise comparison between clusters.
...
```{r}
collected.ranks <- list()
for (comp in names(result.logFC)) { 
    upregulated <- result.logFC[[comp]] > 0
    all.ranks <- rep(Inf, length(upregulated))
    all.ranks[upregulated] <- rank(result.PValue[[comp]][upregulated], ties="first")
    collected.ranks[[comp]] <- all.ranks
}
...
Figure ((heatmapmarkerbrain)) indicates that most of the top markers have strong and consistent upregulation in cells of cluster 1 compared to some or all of the other clusters.
...
Note that many of the markers in Figure ((heatmapmarkerbrain)) are not uniquely expressed in the chosen cluster.
Testing for unique expression tends to be too stringent as it overlooks important genes that are expressed in two or more clusters.
...
A combination of markers can then be chosen to characterize a subpopulation, which is more flexible than trying to find uniquely expressed genes.

It must be stressed that the _p_-values reported here cannot be interpreted as measures of significance.
...
For such analyses, the FDR-adjusted _p_-value can be directly used to define significant genes for each DE comparison, though some care may be required to deal with plate effects [@hicks2015widespread; @tung2016batch; @lun2016overcoming].
