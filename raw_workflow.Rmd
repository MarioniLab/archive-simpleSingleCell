...
We consider this to be risky as it can remove high-quality cells in rare populations.
...
The result of phase assignment for each cell in the HSC dataset is shown in Figure \@ref(fig:phaseplothsc).
...
We define the abundance of each gene as the average count across all cells, with some adjustment for library size differences between cells (Figure \@ref(fig:abhisthsc)).
...
Either of these metrics can be used to filter out low-abundance genes.
The example below demonstrates how we could only retain genes with average counts greater than or equal to 1.
Alternatively, we could retain genes that are expressed in at least _n_ cells.

```{r}
to.keep <- ave.counts >= 1
summary(to.keep)
```

We generally prefer the mean-based filter as it tends to be less aggressive.
A gene will be retained as long as it has sufficient expression in any subset of cells.
In contrast, the "at least _n_" filter depends heavily on the choice of _n_, possibly resulting in the failure to detect rare subpopulations that are present at frequencies below _n_.

Here, we save the two abundance metrics into the `SCESet` object for future use.
We _could_ also directly subset the `SCESet` object by row to remove the low-abundance genes.
However, at this point, we choose to retain all genes in case they are needed later, e.g., for visualization.

```{r}
fData(sce)$ave.counts <- ave.counts
fData(sce)$num.cells <- num.cells
```
...
```{r, warning=FALSE}
sce <- computeSumFactors(sce, subset.row=fData(sce)$ave.counts >= 1)
...
- We set `subset.row` in the code above to remove the low-abundance genes prior to the calculation of size factors.
`computeSumFactors` is robust to the high frequency of zeroes in scRNA-seq data, but it will eventually fail (yielding negative size factors) if too many counts are zero.
A threshold of 1 is usually satisfactory for read count data, while for UMI data, the counts are universally lower so a threshold of 0.1 is used.
...
nzero.exprs <- fData(sce)$ave.count > 0
plotExplanatoryVariables(sce[nzero.exprs,], variables=c("counts_feature_controls_ERCC", 
...
- `trendVar` will automatically ignore genes with mean log-expression values below 0.1 when fitting the trend.
This ensures that low-abundance genes dominated by discreteness do not interfere with the fit.
The default threshold was chosen by assuming Poisson-distributed counts, but can also be manually specified by setting `mean.threshold`.
In most cases, manual specification is not necessary, though lowering the threshold to 0.001-0.01 may improve the fit for very sparse data, e.g., from droplet-based protocols. 
...
We can identify HVGs based on the statistics reported by `decomposeVar`, as shown below.
...
HVGs are strong candidates for further study as they are responsible for the biological heterogeneity in the cell population.
...
We check the distribution of expression values for the top HVGs to ensure that the variance estimate is not driven by one or two outlier cells (Figure \@ref(fig:hvgvioplothsc)).
...
```{r}
var.fit.nospike <- trendVar(sce, use.spikes=FALSE, span=0.2)
...
Even if our assumption does not hold, the use of `decomposeVar` may still be valid if we are willing to accept a change to the null hypothesis.
Instead of testing whether the total variance is greater than the technical noise, we ask whether the total variance of each gene is greater than the average biological variance for the majority of other genes.
This effectively identifies HVGs relative to other genes, which is still a useful result provided that most genes are not differentially expressed across the population.
...
This is useful for denoising prior to downstream procedures that require a gene expression profile for each cell.
...
out5 <- plotTSNE(sce, exprs_values="lowrank", perplexity=5, colour_by="total_features", 
    rand_seed=100, feature_set=chosen) + fontsize + ggtitle("Perplexity = 5")
out10 <- plotTSNE(sce, exprs_values="lowrank", perplexity=10, colour_by="total_features",
    rand_seed=100, feature_set=chosen) + fontsize + ggtitle("Perplexity = 10")
out20 <- plotTSNE(sce, exprs_values="lowrank", perplexity=20, colour_by="total_features",
    rand_seed=100, feature_set=chosen) + fontsize + ggtitle("Perplexity = 20")
...
- For PCA, more components can be shown but are usually less informative (and more difficult to interpret) as they explain less of the variance. 
...
- Putative subpopulations can be computationally defined by clustering, but we do not attempt this here due to the absence of strong substructure.
We will discuss this in more detail for the next data set.
...
```{r topgenehsc, fig.height=9, fig.width=6, fig.cap="Percentage of total counts assigned to the top 50 most highly-abundant features in the brain dataset. For each feature, each bar represents the percentage assigned to that feature for a single cell, while the circle represents the average across all cells. Bars are coloured by the total number of expressed features in each cell, while circles are coloured according to whether the feature is labelled as a control feature."}
plotQC(sce, type = "highest-expression", n=50) + fontsize
```

Gene abundance is quantified by computing the average count across all cells (Figure \@ref(fig:abhistbrain)).
As previously mentioned, the number of transcript molecules will generally be lower than the number of reads generated from such molecules.
...
For example, we might filter out low-abundance genes by setting a minimum threshold of 0.1 on the average count.
(Obviously, the chosen threshold here will be lower than the one recommended for average read counts.)

```{r}
fData(sce)$ave.count <- ave.counts
keep.example <- ave.counts >= 0.1
summary(keep.example)
```
...
sce <- computeSumFactors(sce, cluster=clusters, 
    subset.row=fData(sce)$ave.count >= 0.1)
summary(sizeFactors(sce))
```
...
- We specify high-abundance genes for normalization by using the `subset.row` argument.
As previously mentioned, we use a lower threshold for abundance to reflect the fact that the counts are lower for UMI data.
...
```{r}
var.fit <- trendVar(sce, span=0.4, design=design)
var.out <- decomposeVar(sce, var.fit)
```
...
(It also provides an example where most genes _are_ highly variable, such that fitting a trend to their variances will not recover the technical component.)
...
Again, we check the distribution of expression values for the top 10 HVGs to ensure that they are not being driven by outliers (Figure \@(fig:hvgvioplotbrain)).
...
This yields a low-rank approximation where the technical noise has been removed.

```{r}
sce <- denoisePCA(sce, design=design, value="lowrank", subset.row=var.out$bio > 0)
```
...
```{r pcaplotbrain, fig.cap="PCA plots constructed from the normalized and corrected log-expression values of correlated HVGs for cells in the brain dataset. Each point represents a cell and is coloured according to its expression of the _Neurod6_ (left) or _Mog_ (right).", fig.width=15, fig.height=6}
pca1 <- plotPCA(sce, exprs_values="norm_exprs", colour_by="Neurod6") + fontsize
...
```{r}
chosen.exprs <- assayDataElement(sce, "lowrank")
...
We visualize the cluster assignments for all cells on the _t_-SNE plot in Figure \@(fig:heatmapbrain).
Adjacent cells are generally assigned to the same cluster, indicating that the clustering procedure was applied correctly.

```{r heatmapbrain, message=FALSE, fig.width=7, fig.height=10, fig.cap="_t_-SNE plot of the denoised expression profiles of all cells in the brain data set. Each cell is coloured according to the cluster identity to which it was assigned."}
sce$cluster <- my.clusters
plotTSNE(sce, exprs_values="norm_exprs", colour_by="cluster",
...
```{r}
snn.gr <- buildSNNGraph(sce)
library(igraph)
gr.clusters <- cluster_fast_greedy(snn.gr)
```
