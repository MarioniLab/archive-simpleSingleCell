...
to.check <- rowMeans(exprs(sce)) >= 0.01
plotExplanatoryVariables(sce[to.check,], variables=c("counts_feature_controls_Spike", 
...
- We have applied a gentle filter `to.check` based on the mean of the log-expression values.
This is not strictly necessary from a statistical perspective, but reduces the amount of computational work that has to be performed.
The mean log-expression is used as the filter statistic, as this is independent of the F-statistic (i.e., the ratio of explained variance to the total variance) in a linear modelling framework [@bourgon2010independent].
...
```{r}
to.test <- rowMeans(exprs(sce)) >= 0.1
markers <- findMarkers(sce, my.clusters, design=design, subset.row=to.test)
```
...
- Filtering is applied here for the sake of computational efficiency.
It also improves the trend fit within `r Biocpkg("limma")`, following the (same reasoning)[#trendvar] as described for `min.mean` in `trendVar`.
It also reduces the severity of the multiple testing correction, but this is not a major consideration when the adjusted _p_-values are already compromised.
