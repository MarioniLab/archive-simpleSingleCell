...
We will discuss this in more detail later - formal detection of HVGs is not necessary for feature selection during data exploration.
...
```{r}
high.ab <- fData(sce)$ave.count >= 0.1
clusters <- quickCluster(sce, subset.row=high.ab)
sce <- computeSumFactors(sce, cluster=clusters, subset.row=high.ab)
summary(sizeFactors(sce))
```
...
As previously mentioned, we use a lower threshold of 0.1 to reflect the fact that the counts are lower for UMI data.
