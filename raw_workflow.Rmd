...
Removing genes that are expressed in fewer than _n_ cells protects against genes with outlier expression patterns, i.e., strong expression in only one or two cells. 
Such outliers are typically uninteresting as they can arise from amplification artifacts.
That said, they can also arise from rare populations, which is more biologically relevant. 
...
Here, we consider genes with average counts below 1 to be of low abundance, the identities of which are saved into the `SCESet` object for future use.
...
```{r}
sce$filter.ave <- ave.counts >= 1
summary(sce$filter.ave)
```
...
```{r, warning=FALSE}
sce <- computeSumFactors(sce, sizes=seq(20, 80, 5), subset.row=sce$filter.ave)
...
Also note that we set `subset.row=sce$filter.ave` to remove the low-abundance genes prior to the calculation of size factors.
`computeSumFactors` is robust to the high frequency of zeroes in scRNA-seq data, but it will eventually fail if too many counts are zero.
Obviously, cell-based QC should always be performed prior to normalization, to remove cells with very low numbers of expressed genes.
