...
Size factors are also converted to normalization factors, as previously described.

```{r}
y <- convertTo(sce, fData.col='SYMBOL')
```
...
```{r}
y.sum <- DGEList(summed, genes=fData(sce)[,"SYMBOL",drop=FALSE])[!isSpike(sce),]
...
<!-- This is arguably more useful.
Follow-up on genes that are DE in a particular subpopulation can be done by breaking it down to single cell counts again.
Genes that are DE across the entire group give you nothing beyond what you would have gotten with bulk.
-->
...
examined <- order(res.sum$table$PValue)[1:10]
cur.genes <- fData(sce)$SYMBOL[examined]
all.names <- outer(cur.genes, culture[is.tested], FUN=function(x, y) paste0(x, " (", y, ")"))
par(mar=c(7.1, 4.1, 1.1, 1.1))
boxplot(split(debatched[examined,is.tested], all.names), ylab="Normalized log-expression",
...
```{r}
summary(decideTestsDGE(res.sum))
```
