...
hist(ave.counts, breaks=100, main="", col="grey80", log="x", xlab="Average count")
```
...
smoothScatter(ave.counts, num.cells, xlab="Average count", ylab="Number of cells", log="x")
```

Either of these metrics can be used to filter out low-abundance genes.
The example below demonstrates how we might choose to only retain genes with average counts greater than or equal to 1.
...
We _could_ also directly subset the `SCESet` object by row at this point to remove the low-abundance genes.
However, at this point, we choose to retain all genes in case they are needed later (e.g., for visualization).

```{r}
fData(sce)$ave.count <- ave.count
...
```{r, warning=FALSE}
sce <- computeSumFactors(sce, subset.row=fData(sce)$ave.count >= 1)
...
We assume that biological processes involving coregulated groups of genes will account for the most variance in the data and be represented by earlier principal components (PCs).
This means that random technical noise in each gene will be represented by later PCs.
...
pcs <- denoisePCA(sce, technical=var.fit.des$trend, subset.row=to.keep) 
dim(reducedDimension(pcs)) 
```
...
On a related note, we only show the first two components that contribute most to the variance in Figure ((pcaplothsc)).
