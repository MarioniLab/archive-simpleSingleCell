...
For example, cells can be ordered by pseudotime (e.g., for differentiation) with the `r Biocpkg("monocle")` package [@trapnell2014dynamics], or cell-state hierarchies can be characterized with the `r Biocpkg("sincell")` package [@julia2015sincell].
HVGs can be used in gene set enrichment analyses with packages like `r Biocpkg("topGO")` and `r Biocpkg("goseq")`, to identify biological pathways and processes with heterogeneous activity.
