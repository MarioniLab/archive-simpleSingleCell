...
heatmap.2(heat.vals, col=bluered, symbreak=TRUE, trace='none', cexRow=0.3,
    ...
    breaks=seq(-5, 5, length.out=21), symmbreaks=FALSE)
```

This heatmap can be stored at a greater resolution for detailed inspection later.

```{r, results='hide'}
pdf("brain_heat.pdf", width=10, height=100) # Large 'height' to store all genes
heatmap.2(heat.vals, col=bluered, symbreak=TRUE, trace='none', cexRow=0.3,
    ColSideColors=clust.col[my.clusters], Colv=as.dendrogram(my.tree), 
    keysize=0.5, # Avoid having a very large colour key.
    breaks=seq(-5, 5, length.out=21), symmbreaks=FALSE)
dev.off()
```
...
In this manner, fine-scale structure within each cluster can be explored at greater resolution. 
