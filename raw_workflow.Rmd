---
title: "A step-by-step workflow for low-level analyses of single-cell RNA-seq data with Bioconductor"
author: 
    - name: Aaron T. L. Lun
...
Count data is analyzed to detect highly variable genes (HVGs) that drive heterogeneity across cells in a population, to find correlations between genes and cellular phenotypes, or to identify new subpopulations via dimensionality reduction and clustering. 
...
Note that the use of the spike-in total in `plotExplanatoryVariables` assumes that no quantification is performed (i.e., the amount of cDNA from each library was not equalized prior to pooling and multiplexed sequencing).
...
Thus, for heterogeneous data sets, the spike-in total would no longer be an accurate proxy for the relative technical bias in each sample.
...
There may be a subpopulation of _Fos_ and _Jun_-negative cells, but this is difficult to trust given the small numbers of cells and genes involved.
...
plotExpression(sce, rownames(hvg.out)[1:10], alpha=0.05) + fontsize
```
...
Cells separate into clear clusters in the _t_-SNE plot (Figure ((dimredplotbrain))), corresponding to distinct subpopulations.
...
This is because the former will preserve the effects of the latter on gene expression.
For non-DE normalization, any change in total RNA content will systematically alter the expression of all genes in the non-DE subset, such that it will be treated as bias and removed.
