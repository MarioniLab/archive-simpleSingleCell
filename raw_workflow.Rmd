...
In practice, this strategy is compromised by the small number of spike-in transcripts and the uneven distribution of their abundances.
This makes it difficult to accurately fit a complex trend to the spike-in variances.
...
This is the only option in data sets where no spike-ins were added or not enough spike-in RNA was added to cover the range of means for the endogenous genes.
...
The endogenous trend passes through most of the spike-in variances, indicating that our assumption (that most genes have low levels of biological variability) is valid.
In contrast, the spike-in trend is irregular and systematically understates the variance estimates at intermediate abundances.
This is because the spike-ins themselves are too few in number and their variance estimates are too imprecise for reliable trend fitting.
...
For expression values on the log~2~ scale, this means that gene expression will vary by at least 2-fold around the mean due to biological effects.
...
D.J.M. was supported by a CJ Martin Early Career Research Fellowship from the NHMRC.
