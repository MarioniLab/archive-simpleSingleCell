...
Of particular interest is whether there are confounding technical effects that need to be incorporated into downstream models.
For this data set, the simple experimental design means that there are no plate or batch effects to examine.
Instead, we focus on the total number of expressed features (i.e., the "cellular detection rate") to determine if it needs to modelled [@finak2015mast].

One approach to visualization is to construct a PCA plot from the normalized log-expression values (Figure ((pcaplothsc))).
...
By default, the `plotPCA` function will only use the top 500 genes with the largest variances in the log-expression values.
...
plotPCA(sce, exprs_values="exprs", colour_by="total_features") + fontsize
```
...
Like `plotPCA`, the `plotTSNE` function will use the genes with the largest variances to highlight any heterogeneity.
...
```{r tsneplothsc, fig.cap="_t_-SNE plots constructed from normalized log-expression values using a range of perplexity values. In each plot, each point represents a cell in the HSC data set. Bars represent the coordinates of the cells on each axis. Each cell is coloured according to its total number of expressed features.", fig.width=12, fig.height=6}
set.seed(100)
...
For this data set, the results suggest that we do not need to consider the cellular detection rate in our downstream analyses.
This metric does not associate with any obvious substructure in Figure ((tsneplothsc)), and while it correlates well with the first principal component in Figure ((pcaplothsc)), this accounts for only a small percentage of the total variance.
On a separate note, none of the tested methods are able to separate the into multiple distinct subpopulations.
This is consistent with the presence of a homogeneous population of cells of the same type.
...
This is because the transcriptional program associated with the cell cycle should be mostly conserved across cell types.
...
We examine the expression of the significantly correlated HVGs with a heatmap (Figure ((heatmaphsc))). 
All expression values are mean-centred for each gene to highlight the relative expression between cells.
Weak substructure in the heatmap is consistent with the low number of correlated HVGs.
Nonetheless, a close look suggests that a _H2-Aa_ and _Cd74_-expressing subpopulation exists alongside a _Fos_ and _Jun_-negative subpopulation.
...
heat.out <- heatmap.2(heat.vals, col=bluered, symbreak=TRUE, trace='none', cexRow=0.8)
```

In theory, putative subpopulations can be computationally defined by cutting the dendrogram in `heat.out$colDendrogram` with `cutree` to form clusters.
We do not attempt this here as the number of genes is too low for reliable clustering.
In fact, users should generally treat clustering results with some caution.
It is difficult to maintain statistical rigour to protect against the formation of spurious clusters.
...
Experimental validation of the results is critical to ensure that the putative subpopulations actually exist.
...
To this end, the correlation results can be used directly in follow-up experiments to verify if any substructure is present.
...
Here, we cluster based on Euclidean distances to provide greater sensitivity to differences in expression for low numbers of genes.
...
```{r xheatmaphsc, eval=FALSE, message=FALSE, fig.width=7, fig.height=7, fig.cap="Heatmap of mean-centred normalized log-expression values for correlated HVGs in the HSC data set. Dendrograms are formed by hierarchical clustering on the Euclidean distances between genes (row) or cells (column). Column colours represent the cluster to which each cell is assigned after a tree cut."}
library(gplots)
heat.vals <- norm.exprs - rowMeans(norm.exprs)
...
```{r pcareduxhsc, eval=FALSE, fig.width=10, fig.height=6, fig.cap="PCA (left) and _t_-SNE plots (right) using only the expression values for significantly correlated HVGs in the HSC data set. Cells are coloured according to the level of _H2-Aa_ expression."}
...
Other candidates for removal include pseudogenes or ribosome-associated genes, which might not be relevant for characterising cell function or behaviour but can still interfere with interpretation of the results.
...
This can lead to batch effects due to technical differences in library preparation between chips [@hicks2015widespread; @tung2016batch].
...
The top HVGs are identified based on their biological components.
...
The log-expression values for correlated HVGs are used to cluster cells into putative subpopulations.
We first apply the `removeBatchEffect` function from the `r Biocpkg("limma")` package [@ritchie2015limma] to remove the sex effect.
This ensures that any differences due to sex will not dominate the visualization of the expression profiles.
(Note that, if an analysis method can accept a design matrix, blocking on nuisance factors in the design matrix is preferable to manipulating the expression values with `removeBatchEffect`.
...
We perform hierarchical clustering on the Euclidean distances between cells, using Ward's criterion to minimize the total variance within each cluster.
...
Figure ((heatmapbrain)) shows systematic differences between clusters of cells with distinct patterns of expression.
