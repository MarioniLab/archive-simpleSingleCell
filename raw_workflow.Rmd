...
spike.drop <- isOutlier(sce$pct_counts_feature_controls_ERCC, n=3, type="higher")
```
...
To avoid this, we compute a separate set of size factors for each set of spike-ins.
...
These size factors are stored in the `SCESet` object and will be used instead of the gene-based factors in downstream analyses.

```{r}
sizeFactors(sce, type="ERCC") <- computeSpikeFactors(sce, type="ERCC", sf.out=TRUE)
```
...
In addition, spike-in transcripts will be normalized separately from the endogenous genes if specific size factors are present in `sce`.
...
```{r}
set.seed(100)
var.cor <- correlatePairs(sce, subset.row=top.hvgs[1:200])
...
```{r}
sizeFactors(sce, type="Spike") <- computeSpikeFactors(sce, type="Spike", sf.out=TRUE)
```
...
Alternative approaches to handle cell cycle effects will be discussed in a later section.
...
```{r}
set.seed(100)
var.cor <- correlatePairs(sce, design=design, subset.row=top.hvgs[1:200])
...
We do not have to additionally set spike-in-specific size factors, as these are already being used for all genes and spike-in transcripts.
...
This is more clearly demonstrated with a data set involving different cell types -- namely, mouse embryonic stem cells (mESCs) and fibroblasts [islam2011characterization].
If spike-in size factors were used, the expression values in fibroblasts would be scaled up while expression in mESCs would be scaled down (Figure ((normplotspikemef))).
However, the opposite would occur if deconvolution size factors were used.
This is because fibroblasts contain more endogenous RNA which reduces the relative spike-in coverage (thereby decreasing the spike-in size factors) but increases the coverage of endogenous genes (thus increasing the deconvolution size factors).
...
For example, @islam2011characterization observe a 5-fold difference in total RNA between mESCs and fibroblasts.
Similarly, the total amount of RNA in a cell changes across phases of the cell cycle [@buettner2015computational].
Spike-in normalization will preserve these differences in downstream analyses.

## Blocking on the cell cycle phase

### Using the phase scores from `cyclone`
...
```{r braincyclepca, fig.width=10, fig.height=5, fig.cap="PCA plot of the brain data set, using only genes with annotated functions in cell cycling or division."}
