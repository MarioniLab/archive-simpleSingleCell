...
```{r}
clusters <- quickCluster(sce, subset.row=keep.example)
sce <- computeSumFactors(sce, cluster=clusters, subset.row=keep.example)
summary(sizeFactors(sce))
```
...
for (block in levels(sce$sex)) {
    ...
    var.fit <- trendVar(cur.sce, span=0.4)
...
```{r tsneplotbrain, fig.cap="_t_-SNE plots constructed from the denoised PCs using the cells in the brain dataset. Each point represents a cell and is coloured according to its expression of _Neurod6_ (left) or _Mog_ (right).", fig.width=15, fig.height=6}
...
tsne.out <- plotTSNE(sce, use_reddim=TRUE, colour_by="cluster",
    perplexity=10, rand_seed=100) + fontsize
```
...
fit.block <- trendVar(sce, use.spikes=NA, design=design)
...
The result of this procedure is visualized with some PCA plots in Figure \@ref(pcaplotth2).
