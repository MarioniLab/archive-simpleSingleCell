...
We examine the top DEGs with boxplots of batch-corrected log-expression values (Figure ((deboxplotmesc))).
...
```{r deboxplotmesc, fit.width=10, fig.height=6, fig.cap="Boxplots of batch-corrected normalized log-expression values for the top 10 DEGs in the mESC data set. Points correspond to cells that are more than 1.5 interquartile ranges from the edge of each box."}
sce <- normalize(sce)
...
The boxplots in Figure ((deboxplotmescbulk)) suggest that each of the top genes is exhibiting robust DE across a number of cells.
...
```{r deboxplotmescbulk, fit.width=10, fig.height=6, fig.cap="Boxplots of batch-corrected normalized log-expression values for the top 10 DEGs in the mESC data set, detected by after summing counts for all cells on each plate."}
